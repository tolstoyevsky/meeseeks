...
class MeeseeksCore(Generic[_T]):
    ...
    _domain: str = settings.ROCKET_CHAT_API_V1.split('://')[1].replace('/api/v1', '')
    _certificate: str = 's' if settings.ROCKET_CHAT_API_V1.split('://')[0].endswith('s') else ''
    ...
    async def run(self) -> None:
        ...
        async with websockets.connect(websocket_url) as websocket:
            ...
            self._rtapi: RealTimeAPI = RealTimeAPI(self._request, self._websocket)
            self._restapi: RestAPI = RestAPI(self._headers)
