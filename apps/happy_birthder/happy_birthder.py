...
class HappyBirthder(CommandsMixin, DialogsMixin, MeeseeksCore):
    ...
    async def create_birthday_group(self, user):
        ...
        if settings.CREATE_BIRTHDAY_CHANNELS:
            ...
            group_id = (await self._restapi.get_rooms()).get(birthday_group_name)
            group_users = await self._restapi.get_group_users(birthday_group_name)
            for group_user in group_users:
                if (group_user['username'] not in users_for_mailing.values() and
                        group_user['username'] != settings.USER_NAME):
                    await self._restapi.invite_user_to_group(group_id, group_user['_id'])
    ...
    async def check_dates(self):
        ...
        for user in users_info:
            ...
            if (user['fwd'] is not None and user['fwd'].day == today.day and
                ...
                year_str = 'years' if anniversary == 1 else 'year'
                users_anniversary += (f'\n@{user["name"]} has been a part of our team for '
                                      f'*{anniversary} {year_str}!*')

        if users_without_date and settings.BIRTHDAY_LOGGING_CHANNEL:
            await self._restapi.write_msg(
                settings.PERSONS_WITHOUT_BIRTHDAY_RESPONSE + users_without_date,
                settings.BIRTHDAY_LOGGING_CHANNEL)
