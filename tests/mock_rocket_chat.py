...
class MockRocketChat(BaseHTTPRequestHandler):
    """Contains methods mock server Rocket.Chat. """
    ...
    def _send_response(self, response, status_code=200):
        """Responds to requests on server"""
    ...
    def do_GET(self):  # pylint: disable=invalid-name
        ...
        endpoints = {
            ...
            '/users.list_500/': lambda: self._send_response(INTERNAL_SERVER_ERROR_RESPONSE,
                                                            status_code=500),
        }
...
def run_mock_server():
    """Starts the Rocket.Chat mock server and returns its process and address,
